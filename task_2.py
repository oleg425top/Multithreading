...
def randomizer(random_list, event_for_set):
    ...
    with open(fr'{path}.json', 'w', encoding='utf-8') as file:
        json.dump(random_list, file, ensure_ascii=False, indent=3)
    event_for_set.set()

def simple_number(event_for_wait):
    event_for_wait.wait()
    with open(fr'{path}.json', 'r', encoding='utf-8') as file:
    ...
    with open(r'simple_numbers', 'w', encoding='utf-8') as file:
...
def factorial_of_number(event_for_wait):
    ...
    with open(r'factorial_of_numbers', 'w', encoding='utf-8') as file:
        file.write(str(new_list))
